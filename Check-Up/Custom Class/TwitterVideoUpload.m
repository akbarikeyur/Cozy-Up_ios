...
//        return FALSE;
//    }
    return [self setVideoData:[NSData dataWithContentsOfURL:[NSURL URLWithString:_videoFileName]]];
...
- (BOOL) setVideoData:(NSData *)vidData {
    ...
    NSLog(@"Video size: %lu B = %.1f KB = %.2f MB", (unsigned long)videoData.length, (float)videoData.length / (float)KB, (float)videoData.length / (float)MB);
...
- (void) addVideoChunk:(SLRequest*)request {
    ...
    if (videoData.length > VIDEO_CHUNK_SIZE) {
        ...
        NSLog(@"\tsegment_index %d: loc=%lu len=%lu", segment_index, (unsigned long)range.location, (unsigned long)range.length);
    } else {
